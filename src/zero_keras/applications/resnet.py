...
def ResNet(
    ...
    if (weights == "imagenet") and (weights_name in WEIGHTS_HASHES):
        if include_top:  # pragma: no cover
            file_name = f"{weights_name}_weights_tf_dim_ordering_tf_kernels.h5"  # pragma: no cover
            file_hash = WEIGHTS_HASHES[weights_name][0]  # pragma: no cover
        else:
            file_name = f"{weights_name}_weights_tf_dim_ordering_tf_kernels_notop.h5"  # pragma: no cover
