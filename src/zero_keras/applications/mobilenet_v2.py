...
def MobileNetV2(
    ...
    if input_shape is None and input_tensor is not None:
        ...
        if input_shape is None and not backend.is_keras_tensor(
            ...
            default_size = 224  # pragma: no cover
        elif input_shape is None and backend.is_keras_tensor(
            input_tensor
        ):  # pragma: no cover
            if backend.image_data_format() == "channels_first":  # pragma: no cover
            ...
            if rows == cols and rows in [96, 128, 160, 192, 224]:  # pragma: no cover
                default_size = rows  # pragma: no cover
            else:
                default_size = 224  # pragma: no cover
    ...
    # If input_shape is not None, assume default size.
        ...
        if backend.image_data_format() == "channels_first":  # pragma: no cover
            rows = input_shape[1]  # pragma: no cover
            cols = input_shape[2]  # pragma: no cover
        else:
            rows = input_shape[0]  # pragma: no cover
            cols = input_shape[1]  # pragma: no cover
    ...
    if weights == "imagenet":
        if alpha not in [0.35, 0.50, 0.75, 1.0, 1.3, 1.4]:  # pragma: no cover
            raise ValueError(  # pragma: no cover
        ...
        if rows != cols or rows not in [96, 128, 160, 192, 224]:  # pragma: no cover
            rows = 224  # pragma: no cover
    ...
    # channels.
    if alpha > 1.0:
        last_block_filters = _make_divisible(1280 * alpha, 8)  # pragma: no cover
    else:
    ...
    if weights == "imagenet":
        if include_top:  # pragma: no cover
            model_name = (  # pragma: no cover
                "mobilenet_v2_weights_tf_dim_ordering_tf_kernels"
                f"_{float(alpha)}_{rows}.h5"
            )
            weight_path = BASE_WEIGHT_PATH + model_name  # pragma: no cover
...
def _inverted_res_block(inputs, expansion, stride, alpha, filters, block_id):
    ...
    if in_channels == pointwise_filters and stride == 1:
        return layers.Add(name=f"{prefix}add")([inputs, x])  # pragma: no cover
    return x
