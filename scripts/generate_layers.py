...
with open("tests/test_layers.py", "w") as f:
    ...
    for symbol, sig, doc in layers:
        f.write(f"        '{symbol}',\n")
    f.write("    ]\n")
    f.write("    for name in layer_names:\n")
    f.write("        layer_cls = getattr(layers, name)\n")
    f.write("        sig = inspect.signature(layer_cls.__init__)\n")
    f.write("        kwargs = {}\n")
    f.write("        for param_name, param in sig.parameters.items():\n")
    f.write("            if param_name in ('self', 'kwargs', 'args'): continue\n")
    f.write("            if param.default is inspect.Parameter.empty:\n")
    f.write("                kwargs[param_name] = MagicMock()\n")
    f.write("        layer_cls(**kwargs)\n")
